...
pub fn hex_to_binary(gpa: *std.mem.Allocator, hex: []u8, lsb: bool) !std.ArrayListAligned(u8, null) {
	...
	while(index < hex.len) {
		...
		const KEY_INDEX = std.mem.indexOf(u8, &c.HEX_KEYS, H);
		if(KEY_INDEX == null) {
		...
		const VALUE = c.HEX_VALUES[KEY_INDEX.?];
		const converted_value = try std.fmt.allocPrint(gpa.*, "{s}", .{VALUE});
		try byte.appendSlice(converted_value);

		if(index % 2 != 0) {
			if(lsb) {
				var reversed_converted_value = try s.reverse_string(gpa, byte.items);
...
pub fn int_to_hex(gpa: *std.mem.Allocator, byte: u32) !std.ArrayListAligned(u8, null) {
	...
	while (last_stored_quotient != 0) {
		...
		if (remainder > 9) {
			try return_string.insertSlice(0, c.LETTERS[remainder - 10]);
		} else {
