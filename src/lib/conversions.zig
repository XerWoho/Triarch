...
const LIB_CONSTANTS = @import("constants.zig");
...
pub fn hex_to_binary(gpa: *std.mem.Allocator, hex: []u8, lsb: bool) !std.ArrayList(u8) {
    ...
    while (index < hex.len) {
        ...
        const KEY_INDEX = std.mem.indexOf(u8, &LIB_CONSTANTS.HEX_KEYS, H);
        if (KEY_INDEX == null) {
        ...
        const VALUE = LIB_CONSTANTS.HEX_VALUES[KEY_INDEX.?];
...
pub fn int_to_hex(gpa: *std.mem.Allocator, byte: u32) !std.ArrayListAligned(u8, null) {
    ...
    while (last_stored_quotient != 0) {
        ...
        if (remainder > 9) {
            try return_string.insertSlice(0, LIB_CONSTANTS.HEX_LETTERS[remainder - 10]);
        } else {
...
pub fn binary_to_hex(gpa: *std.mem.Allocator, bytes: []u8) !std.ArrayListAligned(u8, null) {
    const first_conversion = try binary_to_int(bytes, u32);
...
pub fn binary_to_int(bytes: []u8, return_type: type) !return_type {
    var final_int: return_type = 0;

    var index = bytes.len;
    while (index > 0) {
        const pow: u16 = @intCast(bytes.len - index);
        const int = bytes[index - 1 .. index];
